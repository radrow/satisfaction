...
fn execute_solvers(formula: Vec<CNFClause>, num_variables: u32) -> (Option<Assignment>, Option<Assignment>) {
...
proptest! {
    #[test]
    fn only_positive_unit_clauses(num_variables in 1..=MAX_NUM_VARIABLES as u32) {
    ...
    #[test]
    fn only_negative_unit_clauses(num_variables in 1..=MAX_NUM_VARIABLES as u32) {
        let formula = (1..=num_variables)
    ...
    fn only_unit_clauses(signs in vec(weighted(0.5), 1..=MAX_NUM_VARIABLES)) {
        let num_variables = signs.len() as u32;
        let formula = signs.iter()
            ...
            .map(|(variable, sign)| {
                ...
                CNFClause{vars: vec![literal((variable as u32)+1)]}
            }).collect();
    ...
    fn arbitrary_cnf_formula(clauses in vec(vec((1..=MAX_NUM_VARIABLES, weighted(0.5)), 1..=MAX_NUM_LITERALS), 1..=MAX_NUM_CLAUSES)) {
        let num_variables = clauses.iter()
            ...
            .expect("Illegally, there were zero clauses!").len() as u32;
