...
use solver::{CadicalSolver, JeroslawWang, SatisfactionSolver, Solver, CNF, DLCS, DLIS, MOM};
...
fn criterion_benchmark(c: &mut Criterion) {
    let path = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("benches/inputs");

    // Change input directory
    let sat_dir = path.join("sat");
    // Append branching strategy
    create_group_for_solver(
        c,
        "DLIS-I",
        InterruptibleSolverWrapper::from(SatisfactionSolver::new(DLIS)),
        &sat_dir,
    );
