...
pub fn to_formula(field: &Field) -> (CNF, HashMap<TentPlace, VarId>) {
    ...
    let row_set: AxisSet = make_coord_set_by(
    ...
    let ((count_col_form, count_row_form), (neigs_form, (corr_form, _assg_mapping))) =
        rayon::join(
            | | rayon::join(| | make_count_constraints(&field.column_counts, &col_set),
                            | | make_count_constraints(&field.row_counts, &row_set)
            ),
            | | rayon::join(| | make_neighbour_constraints(&neighbour_set),
